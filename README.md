...
- **그래프 라이브러리**: 간결한 DSL을 갖춘 경량 그래프 추상화. 기능 커버리지는 [graaf](https://github.com/bobluppes/graaf)에서, 프론트엔드 API는 [galoisenne](https://github.com/breandan/galoisenne)에서 영감을 받았습니다.
- **알고리즘**: 그래프 알고리즘, 문자열 알고리즘, 계산 기하, 정수론, 고전 DP 패턴.
- **시각화**: 그래프 렌더링과 알고리즘 단계별 애니메이션.

## 그래프 DSL

그래프 모듈은 연산자 기반의 간결한 DSL로 그래프를 구성할 수 있습니다.

```python
from algoforge.core.graph import Vertex
...
방향성을 섞는 경우(예: `u - v >> w`)는 `ValueError`를 발생시켜 그래프 타입이 모호해지지 않게 합니다.
